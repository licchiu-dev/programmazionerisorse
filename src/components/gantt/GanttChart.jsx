...
const LANE_H = 30        // altezza per ogni corsia attività
const LANE_PAD = 8       // padding verticale totale per riga
...
function computeLanes(attivita) {
  ...
  const laneEnds = []     // ultimo dataFine occupato per lane[i]
  const assignments = {}
  ...
  for (const att of sorted) {
    ...
    for (let i = 0; i < laneEnds.length; i++) {
      if (laneEnds[i] < att.dataInizio) {
        assignments[att.id] = i
        laneEnds[i] = att.dataFine
        placed = true
        break
      }
    }
    if (!placed) {
      assignments[att.id] = laneEnds.length
      laneEnds.push(att.dataFine)
    }
  }
...
export default function GanttChart() {
  ...
  // Pre-calcola lanes + altezze per ogni cantiere — unica fonte di verità per il layout
  ...
  // Utility: indice colonna da stringa data
  ...
  const handleActivityMouseDown = useCallback((e, att) => {
    ...
    const onMove = (me) => {
      ...
      // Throttle React state update con rAF
      if (!rafRef.current) {
  ...
  const handleResizeMouseDown = useCallback((e, att) => {
    ...
    const onMove = (me) => {
      ...
      dragStateRef.current = { id: att.id, startIdx: s, endIdx: en, resize: true }
      if (!rafRef.current) {
        rafRef.current = requestAnimationFrame(() => {
          setDragVisual(dragStateRef.current ? { ...dragStateRef.current } : null)
          rafRef.current = null
  ...
  const handleRowMouseDown = useCallback((e, cantiereId) => {
    ...
    drawStateRef.current = { cantiereId, startIdx: col, endIdx: col }
    setDrawVisual({ cantiereId, startIdx: col, endIdx: col })

    const onMove = (me) => {
      const col2 = xToCol(me.clientX - rect.left)
      drawStateRef.current = { cantiereId, startIdx: col, endIdx: col2 }
      if (!rafRef.current) {
        rafRef.current = requestAnimationFrame(() => {
    ...
    const onUp = () => {
      ...
      if (ds) {
        ...
        setModal({ open: true, attivita: null, cantiereId, dataInizio: indexToDate(s), dataFine: indexToDate(en) })
      }
    }
    window.addEventListener('mousemove', onMove)
    window.addEventListener('mouseup', onUp)
    ...
    <div className="flex flex-col h-full">
      ...
      <div id="gantt-printable" ref={scrollRef} className="flex-1 overflow-auto mx-5 my-3 bg-white rounded-xl border border-gray-200 shadow-sm">
        <div style={{ width: LEFT_W + totalGridWidth, minWidth: '100%' }}>
          ...
          {/* ── Rows ── */}
          {rowData.map(({ cantiere, atts, assignments, height }) => (
            <div key={cantiere.id} className="flex border-b border-gray-100" style={{ height }}>

              {/* Sticky left label */}
              <div
                className="sticky left-0 z-20 bg-primary-50 border-r border-primary-100 flex items-center gap-2 px-3 shrink-0"
              ...
              >
                ...
                {/* Draw preview */}
                {drawVisual?.cantiereId === cantiere.id && (
                  <div
                    className="absolute rounded bg-primary-300/60 border border-primary-400 pointer-events-none z-20"
                    style={{
                ...
                {atts.map(att => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {att.dipendentiIds.length > 1 && <span className="ml-1 text-white/70 shrink-0">+{att.dipendentiIds.length - 1}</span>}
                      {/* Resize handle */}
                      <div
        ...
        <div className="sticky bottom-0 border-t border-gray-100 px-5 py-2 flex items-center gap-6 flex-wrap bg-gray-50/95 backdrop-blur-sm text-xs text-gray-500">
          <span className="flex items-center gap-1.5"><span className="w-8 h-3 rounded bg-primary-400 inline-block" />Attività</span>
